...
let ff graph sc sk =
  (* Initialyzing the (int*int) graph *)
  ...
  let rec ffrec gr = 
    ...
    try 
      (* We try to find an augmenting path here.
         - If there is at least one existing, the exceptin is raised and we call the function again.
         - If there is no augmenting path left, we return the graph
      *)
      let _ = dfs ecart sc sk in
      (* We do not bind a value here, so we use the generic _ to just run the dfs and raise the exception *)
      gr
